...
# We count the maximum number of occurances for each prime as a factor,
# the product of these maximums will be the scm

primeCardinality = {prime:0 for prime in generate_primes_up_to(20)}

for number in xrange(1, 21):
    for prime, val in primeCardinality.iteritems():
        count = 0
        while not number % prime:
            count += 1
            number /= prime
            primeCardinality[prime] = max(primeCardinality[prime], count)
        if number == 1:
            break

print reduce(operator.mul,
            (prime ** val for prime, val in primeCardinality.iteritems()))
